...
`Kernel` is the closed‑source heart of the Razor trading engine. It contains all trading logic, including backtesting, live trading, genetic optimisation, brokers, hook infrastructure, telemetry, and messaging. This project is the **core runtime** that executes strategies, evaluates fitness, and manages account state.

This project is **not** part of the public SDK. It is referenced by `Engine` and is never exposed directly to extension developers.

---

## Key Components

### 1. Backtesting (`Kernel.Backtesting`)
...
### 4. Configuration (`Kernel.Configuration`)
...
### 7. Indicator Registry (`Kernel.Indicators`)
...
### 10. Neural Networks (`Kernel.NeuralNetworks`)
...
### 11. Optimisation (`Kernel.Optimization`)
...
| Dependency | Purpose |
|------------|---------|
| `Sdk` | Provides all public contracts, domain types, and helpers. |
| `Shared` | Provides memory‑mapped tick lists and binary file mapping. |
...
- `Engine` (via `KernelService` facade)
- `Kernel.UnitTests` and `Kernel.IntegrationTests`
...
*This README is intended for Razor core developers. For extension development, see the [Sdk README](src/Sdk/README.md).*