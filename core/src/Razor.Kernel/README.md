...
`Razor.Kernel` is the closed‑source heart of the Razor trading engine. It contains all trading logic, including backtesting, live trading, genetic optimisation, brokers, hook infrastructure, telemetry, and messaging. This project is the **core runtime** that executes strategies, evaluates fitness, and manages account state.

This project is **not** part of the public SDK. It is referenced by `Razor.Engine` and is never exposed directly to extension developers.

---

## Key Components

### 1. Backtesting (`Razor.Kernel.Backtesting`)
...
### 2. Brokers (`Razor.Kernel.Brokers`)
...
### 3. Clock System (`Razor.Kernel.Clock`)
...
### 4. Configuration (`Razor.Kernel.Configuration`)
...
### 6. Hook System (`Razor.Kernel.Hooks`)
...
### 7. Indicator Registry (`Razor.Kernel.Indicators`)
...
### 8. Messaging (`Razor.Kernel.Messaging`)
...
### 9. Metrics (`Razor.Kernel.Metrics`)
...
### 10. Neural Networks (`Razor.Kernel.NeuralNetworks`)
...
### 11. Optimisation (`Razor.Kernel.Optimization`)
...
### 12. Reporting (`Razor.Kernel.Reporting`)
...
### 13. Telemetry (`Razor.Kernel.Telemetry`)
...
| Dependency | Purpose |
|------------|---------|
| `Razor.Sdk` | Provides all public contracts, domain types, and helpers. |
| `Razor.Shared` | Provides memory‑mapped tick lists and binary file mapping. |
...
- `Razor.Engine` (via `KernelService` facade)
- `Razor.Kernel.UnitTests` and `Razor.Kernel.IntegrationTests`
...
*This README is intended for Razor core developers. For extension development, see the [Razor.Sdk README](src/Razor.Sdk/README.md).*