...
`Engine` is the headless executable that hosts the Razor Kernel, manages extensions, and communicates with Razor Cloud via a persistent encrypted WebSocket connection. It is the user‑facing binary that traders and quants deploy on their infrastructure.
...
### 2. Communication (`Engine.Communication`)
...
### 4. Extensions (`Engine.Extensions`)
...
### 5. Kernel Facade (`Engine.Kernel`)

`KernelService` bridges Engine commands to `Kernel` operations.
...
### 6. Command Dispatcher (`Engine.Management.Commands`)
...
### 7. Task Manager (`Engine.Management.Tasks`)
...
### 8. Cron & Scheduling (`Engine.Management.Scheduling`)
...
### 9. BehaviorRecorder (`Engine.Services.BehaviorRecorder`)
...
### 10. Self‑Update (`Engine.Services.Update`)
...
| `Shared` | Memory‑mapped tick lists. |
| `Kernel` | Core trading logic. |
...
1. Publish to a folder:
   ```bash
   dotnet publish src/Engine --configuration Release --output ./publish
   ```
...
# With credentials
./Engine --auth=username,password,apikey

# As a service (Windows)
Engine.exe --service --auth=username,password,apikey

# As a service (Linux)
./Engine --service --auth=username,password,apikey
```
...
- **Unit tests:** `Sdk.UnitTests` (200+ tests covering SDK contracts).
- **Integration tests:** `Sdk.IntegrationTests` (covers file I/O, random, calculators, hooks).
...
*This README is intended for Razor core developers and system administrators. For extension development, see the [Sdk README](src/Sdk/README.md).*