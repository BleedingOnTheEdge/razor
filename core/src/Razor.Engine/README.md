...
`Razor.Engine` is the headless executable that hosts the Razor Kernel, manages extensions, and communicates with Razor Cloud via a persistent encrypted WebSocket connection. It is the user‑facing binary that traders and quants deploy on their infrastructure.
...
### 2. Communication (`Razor.Engine.Communication`)
...
### 3. Core Services (`Razor.Engine.Core`)
...
### 4. Extensions (`Razor.Engine.Extensions`)
...
### 5. Kernel Facade (`Razor.Engine.Kernel`)

`KernelService` bridges Engine commands to `Razor.Kernel` operations.
...
### 6. Command Dispatcher (`Razor.Engine.Management.Commands`)
...
### 7. Task Manager (`Razor.Engine.Management.Tasks`)
...
### 8. Cron & Scheduling (`Razor.Engine.Management.Scheduling`)
...
### 9. BehaviorRecorder (`Razor.Engine.Services.BehaviorRecorder`)
...
### 10. Self‑Update (`Razor.Engine.Services.Update`)
...
| Dependency | Purpose |
|------------|---------|
| `Razor.Sdk` | Public contracts. |
| `Razor.Shared` | Memory‑mapped tick lists. |
| `Razor.Kernel` | Core trading logic. |
...
1. Publish to a folder:
   ```bash
   dotnet publish src/Razor.Engine --configuration Release --output ./publish
   ```
...
# With credentials
./Razor.Engine --auth=username,password,apikey

# As a service (Windows)
Razor.Engine.exe --service --auth=username,password,apikey

# As a service (Linux)
./Razor.Engine --service --auth=username,password,apikey
```
...
- **Unit tests:** `Razor.Sdk.UnitTests` (200+ tests covering SDK contracts).
- **Integration tests:** `Razor.Sdk.IntegrationTests` (covers file I/O, random, calculators, hooks).
...
*This README is intended for Razor core developers and system administrators. For extension development, see the [Razor.Sdk README](src/Razor.Sdk/README.md).*